...
static NSString *kDefaultCacheName = @"tbCache";
static NSString *kEtagCacheDirectoryName = @"lcache";

static TBURLCache *gSharedCache = nil;
...
- (id)initWithName:(NSString *)name {
    ...
    if (self) {
        _name = [name copy];
        _cachePath = [TBURLCache cachePathWithName:name];
        _invalidationAge = TB_DEFAULT_CACHE_INVALIDATION_AGE;
...
///////////////////////////////////////////////////////////////////////////////////////////////////
+ (TBURLCache *)cacheWithName:(NSString *)name {
    if (nil == gNamedCaches) {
        gNamedCaches = [[NSMutableDictionary alloc] init];
    }
    TBURLCache *cache = [gNamedCaches objectForKey:name];
    if (nil == cache) {
        cache = [[TBURLCache alloc] initWithName:name];
...
///////////////////////////////////////////////////////////////////////////////////////////////////
+ (TBURLCache *)sharedCache {
 
    @synchronized (self) {
        if (nil == gSharedCache) {
            gSharedCache = [[TBURLCache alloc] init];
        }
    }
    return gSharedCache;
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
+ (void)setSharedCache:(TBURLCache *)cache {
...
- (BOOL)imageExistsFromBundle:(NSString *)URL {
    NSString *path = TBPathForBundleResource([URL substringFromIndex:9]);
    NSFileManager *fm = [NSFileManager defaultManager];
...
- (BOOL)imageExistsFromDocuments:(NSString *)URL {
    NSString *path = TBPathForDocumentsResource([URL substringFromIndex:12]);
    NSFileManager *fm = [NSFileManager defaultManager];
    return [fm fileExistsAtPath:path];
}
...
- (NSData *)dataForURL:(NSString *)URL {
    return [self dataForURL:URL expires:TB_CACHE_EXPIRATION_AGE_NEVER timestamp:nil];
}
...
- (BOOL)hasImageForURL:(NSString *)URL fromDisk:(BOOL)fromDisk {
    ...
    if (!hasImage && fromDisk) {
        if (TBIsBundleURL(URL)) {
            hasImage = [self imageExistsFromBundle:URL];
            if (!hasImage) {
                hasImage = [self imageExistsFromBundle:[TBURLCache doubleImageURLPath:URL]];
            }

        } else if (TBIsDocumentsURL(URL)) {
            hasImage = [self imageExistsFromDocuments:URL];
            if (!hasImage) {
                hasImage = [self imageExistsFromDocuments:[TBURLCache doubleImageURLPath:URL]];
            }
...
- (void)removeAll:(BOOL)fromDisk {
    ...
    if (fromDisk) {
        ...
        [TBURLCache createPathIfNecessary:_cachePath];
    }
}
